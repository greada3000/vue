  ...
  <header class="site-header">
    <div class="site-header__inner">
      ...
      <form class="site-search" @submit.prevent="submitSearch">
        <button class="site-search__submit" type="submit" aria-label="搜索"><el-icon><Search /></el-icon></button>
        <input v-model="searchValue" placeholder="搜索灵感与同好" aria-label="搜索内容" @keyup.enter="submitSearch" />
      </form>

      <div class="site-actions">
        <el-button class="icon-button" circle aria-label="通知"><el-icon><Bell /></el-icon></el-button>
...
export default {
  ...
  computed: {
    isLoggedIn() { return this.$store.getters.getLoginStatus },
    user() { return this.$store.getters.getUser || {} },
    username() { return this.user.username || '社区成员' },
    avatarText() { return this.username.slice(0, 1).toUpperCase() },
    profileRoute() { return `/profile/${this.user.name || this.user.userId || 'me'}` }
  },
  watch: { keyword(value) { this.searchValue = value || '' } },
  methods: {
    submitSearch() {
