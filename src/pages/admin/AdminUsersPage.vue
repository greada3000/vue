            ...
            <el-table-column label="操作">
              <template #default="scope">
                ...
                <el-tooltip class="item" effect="dark" content="用户主页" placement="top" :enterable="false">
                  <el-button type="warning" @click="touserhome(scope.row.userId)">
                    <el-icon>
                      <SearchIcon />
                    </el-icon>
                  </el-button>
                ...
                <el-tooltip class="item" effect="dark" content="删除用户" placement="top" :enterable="false">
                  <el-button type="danger" @click="removeUserById(scope.row.userId)">
                    <el-icon>
                      <DeleteIcon />
                    </el-icon>
                  </el-button>
...
export default {
  ...
  methods: {
    createUser(user) {
      this.$api.users.create(user).then((resp) => {
        let data = resp.data;
        if (data.success) {
          this.loginForm = {};
    ...
    getUserList() {
      this.$api.users.search(this.queryInfo).then((resp) => {
        let data = resp.data;
        if (data.success) {
          this.loginForm = {};
          // console.log(data);
    ...
    edit() {
      this.$api.users.update(this.editForm).then((resp) => {
        let data = resp.data;
        if (data.success) {
          this.editForm = {};
          this.getUserList();
